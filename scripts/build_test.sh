...
do
    ...
    go test -shuffle=on ${race:-} -timeout="${TIMEOUT:-600s}" -coverprofile=coverage.out -covermode=atomic "$@" $(go list ./... | grep -v github.com/mev-zone/coreth-validator/tests) | tee test.out || command_status=$?
